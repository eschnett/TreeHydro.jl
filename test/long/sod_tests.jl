...
#
# The boundary hook's ghosts and the arrival assertion — the two claims that
# cost almost nothing — stay in the short tier, in `test/sod_tests.jl`, which
# also defines the `sod_ops` this file runs on.
