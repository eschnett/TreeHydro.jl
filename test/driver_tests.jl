...
@testset "The derived buffer width is what tracks the waves: D=1" begin
    ...
    # The guard itself: a cadence too slow for the cap is refused by
    # `refinement_buffer` naming the constraint, rather than silently
    # producing a margin that cannot be recruited.
...
@testset "The CFL recheck fires when the signal outgrows the step it was sized for" begin
    # Two halves of the step-4 amendment. The first is arithmetic: the
    # recheck is a pure function of five numbers and either the step is
    # inside the condition or it is not. The second is the reason the
    # headroom is a case parameter at all — a Riemann problem's fastest
    # signal is not in its initial data, so a driver that measured λ once
    # per chunk and believed it would run the first chunk of *every* shock
    # case at nearly twice the CFL number it asked for.
    ...
    # And the measurement that sizes the parameter. Sod's initial data
...
@testset "The driver refuses a working type the case was not built at" begin
    # A leading `T` is this package's convention on every driver, and a
    # `HydroCase` already carries one: its extents, its states and its speed
    # headroom are all stated in it, so the type is chosen when the case is
    # built and not when it is run. Asking for a different one is a mistake
    # worth naming — the message says which two types disagree and what to
    # do — rather than a `MethodError` from a signature nobody reads.
