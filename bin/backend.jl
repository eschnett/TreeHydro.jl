...
#
# Shared by both viewers, because both take the same flag and there is
...
#
# Copied from TreeWave's `bin/backend.jl` rather than depended on, as
# `src/precision.jl` and `src/device.jl` are: the three packages share
# patterns, not code.
...
function resolvebackend(name::AbstractString)
    ...
    Base.identify_package(String(pkg)) === nothing && error(
        "--backend=$name needs $pkg in this environment, which is deliberately \
         not a dependency of TreeHydro. Run the script against a project that \
         has it:\n\n    julia --project=/tmp/thgpu -e 'using Pkg; \
...
measured in TreeWave and neither is specific to a wave equation:
