...
# close through those -- are `MethodError`s at `Float32x2` while working at
# `Float32` and `Float64`. Neither is a physics decision, so neither belongs
# spelled out at the call site.
#
# Copied from TreeWave rather than depended on: the two packages are siblings,
# both downstream of TreeAMR, and neither is the other's library.
#
...
The periodic cases — the entropy wave and the Kelvin–Helmholtz instability —
are where this is used: an exact solution evaluated at `x - v t` has to be
brought back into the box before it is compared with the run.
"""
...
`x` as a `Float64` — the bridge to the two host reference codes, the exact
Riemann solver and the Sedov similarity quadrature, which compute in
`Float64` whatever type the run itself uses (see "Precision" in `CODE.md`).
